...
def refine_pin_detection_adjusted(img):
    ...
    distance = 0
    distance2 = 0
    img = adjust_brightness_contrast(img, brightness=1, contrast=22)
    sliced_image = img[450:720, 910:1370]
    ...
    upper_white = np.array([255, 254, 254])

    # Convert BGR to HSV
    hsv = cv2.cvtColor(sliced_image, cv2.COLOR_BGR2HSV)

    # Define range of green color in HSV
    lower_green = np.array([45, 40, 35])
    ...
    if len(green_pin_centers) != 0:
        if distance >= 100+magnitude and distance <= 135+magnitude and distance2 < 100:
            cv2.putText(contour_frame, f'A', (280, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2, cv2.LINE_AA)
            print("A")
            classify_found = "1"
            comfirm = 1
            ...
            #time.sleep(2)
            #GPIO.output(13,True)
        elif distance > 135+magnitude and distance <= 162+magnitude:
            cv2.putText(contour_frame, f'B', (280, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2, cv2.LINE_AA)
            print("B")
            classify_found = "2"
            comfirm = 1
            #GPIO.output(13,False)
            ...
            #time.sleep(2)
            #GPIO.output(13,False)
        elif distance > 162+magnitude and distance <= 195+magnitude:
            cv2.putText(contour_frame, f'C', (280, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2, cv2.LINE_AA)
            print("C")
            classify_found = "3"
            comfirm = 1
        elif distance > 40+magnitude and distance <= 77+magnitude:
            cv2.putText(contour_frame, f'D', (280, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2, cv2.LINE_AA)
            print("D")
            classify_found = "5"
            comfirm = 1
        elif distance > 77+magnitude and distance <= 110+magnitude:
            cv2.putText(contour_frame, f'E', (280, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2, cv2.LINE_AA)
            print("E")
            classify_found = "6"
            comfirm = 1
        elif distance > 110+magnitude and distance <= 140+magnitude and distance2 > 100 and distance2 < 300:
            cv2.putText(contour_frame, f'F', (280, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2, cv2.LINE_AA)
            print("F")
            classify_found = "7"
            comfirm = 1
